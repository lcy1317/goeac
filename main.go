...
func main() {
	...
	//		select {
	//		case <-ticker.C:
	//			fmt.Printf("%s: Speed %d lotterys per second, total: %d\n", time.Now().Format("2006-01-02 15:04:05"), count, totalCount)
	//			count = 0 // 重置每秒计数器
	//		case c := <-countChan:
	//			count += c
	//			totalCount += c // 增加总计数器
	...
	//}()
	//
	//// 创建指定数量的goroutine来并行运行buyLottery
	//for i := 0; i < *threadCount; i++ {
	//	go func() {
	//		for {
	//			buyLottery(addressesSet)
	//			countChan <- 1 // 发送计数信号
	//		}
	//	}()
	//}
	//
	// 主goroutine继续运行，直到程序被外部信号中断
	select {}
